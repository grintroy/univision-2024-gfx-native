async function load(project, version, creditsObject) {
	async function gotCredits(data) {
		...
		fonts =
			typeof globalFonts !== "undefined"
				? [...new Set([...globalFonts, ...data.fonts])]
				: data.fonts;
	...
	if (typeof Cookies !== "undefined") {
		Cookies.set("project", currentProject, {
			secure: true,
			SameSite: "Lax"
		});
	}
	let versionString = String(
		$("#loadFile").find(":selected").data("versions")
	);
	let versions = [];
	...
	if (typeof creditsObject !== "undefined") {
		...
		await $.get(`save?project=${currentProject}&version=${currentVersion}`)
			...
			.fail(function (data) {
				...
				alert(
					"Couldn't get requested file: " +
						JSON.stringify(returnData, "", 4)
				);
			});
	}
	loaded = true;
...
async function buildCredits(content) {
	...
	content.forEach((article) => {
		const name =
			typeof article.name !== "undefined" ? article.name : article.type;
		const trackWidth =
			typeof article.trackWidth !== "undefined"
				? article.trackWidth
				: 1152;
		const trackWidthVW = (trackWidth / 1920) * 100;
		const trackAlign =
			typeof article.trackAlign !== "undefined" ? article.trackAlign : 9;
		const backgroundImage =
			typeof article.backgroundImage !== "undefined"
				? article.backgroundImage
				: "None";
		const backgroundAlign =
			typeof article.backgroundAlign !== "undefined"
				? article.backgroundAlign
				: false;
      ...
      data-duration="${article.duration}">
		...
		$content.css(
			"padding-left",
			`calc(${trackAlign - 1} * (100% - ${trackWidthVW}vw)/16)`
		);
		$content.css(
			"padding-right",
			`calc(${17 - trackAlign} * (100% - ${trackWidthVW}vw)/16)`
		);
		$content.css(
			"--background-position-x",
			`calc(${100 * ((trackAlign - 1) / 16)}%)`
		);
		if (backgroundImage == "None") {
			...
			const path = template
				? "../"
				: `../${sitePath}saves/${currentProject}/`;
			$content.css(
				"--background-image",
				`url('${path}/${backgroundImage}')`
			);
		}
		active = "";
		$cont.append($content);
...
function renderBlocks(blocks) {
	...
	blocks.forEach((block) => {
		let style = "";
		if (
			typeof block.settings !== "undefined" &&
			Object.values(block.settings).length > 0
		) {
			style = 'style="';
		...
		const roleFlip =
			block.roleFlipped == true ? "data-rolealign='true'" : "";
		const nameFlip =
			block.nameFlipped == true ? "data-namealign='true'" : "";
...
function renderContent(content) {
	...
	let style = "";
	if (
		typeof content.settings !== "undefined" &&
		Object.values(content.settings).length > 0
	) {
		style = 'style="';
	...
	switch (content.type) {
		...
		case "names": {
			const flipped =
				content.namesFlipped == true ? "data-flipped='true'" : "";
			const roleFlip =
				content.roleFlipped == true ? "data-rolealign='true'" : "";
			const nameFlip =
				content.nameFlipped == true ? "data-namealign='true'" : "";
		...
		case "image": {
			...
			const path = template
				? "assets/img/credits/"
				: `${sitePath}saves/${currentProject}/`;
			subHtml += `<figure ${style} class="content imageCont" data-type='${content.type}'><img class='image' src='${path}/${content.image}' style='max-height: ${height}em'></figure>`;
			break;
		}
		case "spacing":
	...
	switch (content.type) {
		...
		case "image": {
			...
			if (imgClass.indexOf("em") !== -1) {
				content.imageHeight = imgClass.substring(
					12,
					imgClass.indexOf("em")
				);
			}
			...
			if (image == "../../../img/Placeholder.jpg") {
				...
				const search = `saves/${currentProject}/`;
...
function getStylesObject(element, type = "none") {
	...
	cssArray.forEach((style) => {
		...
		if (
			!(type == "spacing" && prop == "height") &&
			!(type == "image" && prop == "max-height")
		) {
